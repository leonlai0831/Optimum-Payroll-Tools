...
// Bands carry a client-only stable key so React reconciles rows by identity, not
// by array index — removing a middle band must not shift a focused input onto its
// neighbour. The `_key` is stripped before the config is persisted. The key only
// needs to be unique within a list, so a plain monotonic counter suffices.
type KeyedBand = CommissionConfig["bands"][number] & { _key: string };
...
export function CommissionSettingsForm({
  ...
  function addBand() {
    ...
    setCfg((c) => ({ ...c, bands: [...c.bands, { minCount, maxCount: null, rate: 0, _key: nextBandKey() }] }));
  }
  ...
  async function save() {
    setSaving(true);
    try {
      // Strip the client-only `_key` before persisting (RateBand is exactly
      // minCount/maxCount/rate — TS flags this if the shape ever changes).
      const payload: CommissionConfig = {
        ...cfg,
        bands: cfg.bands.map((b) => ({ minCount: b.minCount, maxCount: b.maxCount, rate: b.rate })),
      };
    ...
    <div className="space-y-4">
      <Card className="p-4">
        ...
        <MobileCards className="mt-3">
          {cfg.bands.map((b, i) => (
            <div key={b._key} className="flex items-end gap-2 py-3 first:pt-0">
