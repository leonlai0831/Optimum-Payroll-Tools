 ...
 * workbook); this module turns that grid into `{ email, displayName }` rows.
 *
 * Header handling is forgiving so an operator can paste an export from anywhere:
 *  - If the first non-empty row names an "email" column, columns are mapped by
 *    header (email + a name/nickname column, in any order), data starts below it.
 *  - Otherwise there is no header: column 0 is the email, column 1 the name —
 *    matching the old paste format (`email, name`).
 * Rows without an email cell are dropped; whitespace is trimmed.
 */

export type ParsedUserRow = { email: string; displayName: string };
...
/** Pick the name column from a header row: prefer an explicit display
 *  name/nickname, then a plain "name", never the email column itself. */
function findNameColumn(header: string[], emailCol: number): number {
  ...
  for (const re of [/display\s*name|nick\s*name|nickname/, /\bname\b/, /name/]) {
    const i = prefer(re);
    if (i !== -1) return i;
  }
  return -1;
...
export function rowsFromGrid(grid: Cell[][]): ParsedUserRow[] {
  ...
  for (const row of body) {
    ...
    if (!email) continue;
    const displayName = nameCol === -1 ? "" : cellText(row[nameCol]);
